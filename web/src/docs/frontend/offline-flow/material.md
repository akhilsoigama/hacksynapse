# Material Module - Complete Flow (Offline/Online & Data Isolation)

## Objective
Implement robust offline reading, offline data creation (mutation), and strict **Data Isolation (Tenant, Ownership & ID-Wise Access)** for the `material` module.
...
Jab bhi koi user kisi specific record ko uske **ID** se access karega (e.g., `/api/material/:id`), to backend ye verify karega:
...
- Offline mode me, frontend `getMaterialDB()` array filter karega: 
...
### A. IndexedDB Setup
**File:** `src/indexDB/material.ts`
- Functions: `getMaterialDB()`, `setMaterialDB(data)`, `clearMaterialDB()`, and `getMaterialByIdDB(id)`.
- Schema: `src/indexDB/db.ts` mein `material` table (e.g. `++id, uuid, instituteId, departmentId, createdBy`).

### B. Fetch Hook
**File:** `src/action/material.ts`
- Online hone par API se filtered data aayega.
- Offline hone par `getMaterialDB()` ko call karein aur tenant/ID based filter laga kar state update karein.
...
2. **Offline Check:** Agar offline hai, to record `material_sync_queue` me jayega. UI turant update hoga.
3. **Online Mode:** Seedhe `POST /api/material` hit hoga.

### C. Background Sync Logic
**File:** `src/hooks/useMaterialOfflineSync.ts`
- `online` event fire hone par, `material_sync_queue` se pending tasks nikal kar bulk me `POST /api/material/sync` par bheje jayenge.
...
- `GET /api/material`
- `GET /api/material/:id` (ID-Wise Access)
- `PUT /api/material/:id` (ID-Wise Update)
- `DELETE /api/material/:id` (ID-Wise Delete)

**Logic Execution in `MaterialController`:**
...
## Checklist for Material Module
- [ ] Enforce ID-Wise access control (`show`, `update`, `delete`) based on ownership and institute boundary in backend.
- [ ] Implement Offline ID-wise fetch fallback: `getMaterialByIdDB(id)` verifying tenant scope.
