# AssignmentUpload Module - Complete Flow (Offline/Online & Data Isolation)

## Objective
Implement robust offline reading, offline data creation (mutation), and strict **Data Isolation (Tenant, Ownership & ID-Wise Access)** for the `assignmentUpload` module.
...
Jab bhi koi user kisi specific record ko uske **ID** se access karega (e.g., `/api/assignmentUpload/:id`), to backend ye verify karega:
...
- Offline mode me, frontend `getAssignmentUploadDB()` array filter karega: 
...
### A. IndexedDB Setup
**File:** `src/indexDB/assignmentUpload.ts`
- Functions: `getAssignmentUploadDB()`, `setAssignmentUploadDB(data)`, `clearAssignmentUploadDB()`, and `getAssignmentUploadByIdDB(id)`.
- Schema: `src/indexDB/db.ts` mein `assignmentUpload` table (e.g. `++id, uuid, instituteId, departmentId, createdBy`).

### B. Fetch Hook
**File:** `src/action/assignmentUpload.ts`
- Online hone par API se filtered data aayega.
- Offline hone par `getAssignmentUploadDB()` ko call karein aur tenant/ID based filter laga kar state update karein.
...
```typescript
assignmentUpload_sync_queue: '++id, uuid, action, status, instituteId, departmentId, createdBy'
```
...
2. **Offline Check:** Agar offline hai, to record `assignmentUpload_sync_queue` me jayega. UI turant update hoga.
3. **Online Mode:** Seedhe `POST /api/assignmentUpload` hit hoga.

### C. Background Sync Logic
**File:** `src/hooks/useAssignmentUploadOfflineSync.ts`
- `online` event fire hone par, `assignmentUpload_sync_queue` se pending tasks nikal kar bulk me `POST /api/assignmentUpload/sync` par bheje jayenge.
...
**Routes:**
- `POST /api/assignmentUpload/sync`
- `GET /api/assignmentUpload`
- `GET /api/assignmentUpload/:id` (ID-Wise Access)
- `PUT /api/assignmentUpload/:id` (ID-Wise Update)
- `DELETE /api/assignmentUpload/:id` (ID-Wise Delete)

**Logic Execution in `AssignmentUploadController`:**
...
## Checklist for AssignmentUpload Module
- [ ] Enforce ID-Wise access control (`show`, `update`, `delete`) based on ownership and institute boundary in backend.
- [ ] Implement Offline ID-wise fetch fallback: `getAssignmentUploadByIdDB(id)` verifying tenant scope.
