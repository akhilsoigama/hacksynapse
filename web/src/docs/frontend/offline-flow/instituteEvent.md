# InstituteEvent Module - Complete Flow (Offline/Online & Data Isolation)

## Objective
Implement robust offline reading, offline data creation (mutation), and strict **Data Isolation (Tenant, Ownership & ID-Wise Access)** for the `instituteEvent` module.
...
Jab bhi koi user kisi specific record ko uske **ID** se access karega (e.g., `/api/instituteEvent/:id`), to backend ye verify karega:
...
- Offline mode me, frontend `getInstituteEventDB()` array filter karega: 
...
### A. IndexedDB Setup
**File:** `src/indexDB/instituteEvent.ts`
- Functions: `getInstituteEventDB()`, `setInstituteEventDB(data)`, `clearInstituteEventDB()`, and `getInstituteEventByIdDB(id)`.
- Schema: `src/indexDB/db.ts` mein `instituteEvent` table (e.g. `++id, uuid, instituteId, departmentId, createdBy`).

### B. Fetch Hook
**File:** `src/action/instituteEvent.ts`
- Online hone par API se filtered data aayega.
- Offline hone par `getInstituteEventDB()` ko call karein aur tenant/ID based filter laga kar state update karein.
...
```typescript
instituteEvent_sync_queue: '++id, uuid, action, status, instituteId, departmentId, createdBy'
```
...
2. **Offline Check:** Agar offline hai, to record `instituteEvent_sync_queue` me jayega. UI turant update hoga.
3. **Online Mode:** Seedhe `POST /api/instituteEvent` hit hoga.

### C. Background Sync Logic
**File:** `src/hooks/useInstituteEventOfflineSync.ts`
- `online` event fire hone par, `instituteEvent_sync_queue` se pending tasks nikal kar bulk me `POST /api/instituteEvent/sync` par bheje jayenge.
...
**Routes:**
- `POST /api/instituteEvent/sync`
- `GET /api/instituteEvent`
- `GET /api/instituteEvent/:id` (ID-Wise Access)
- `PUT /api/instituteEvent/:id` (ID-Wise Update)
- `DELETE /api/instituteEvent/:id` (ID-Wise Delete)

**Logic Execution in `InstituteEventController`:**
...
## Checklist for InstituteEvent Module
- [ ] Enforce ID-Wise access control (`show`, `update`, `delete`) based on ownership and institute boundary in backend.
- [ ] Implement Offline ID-wise fetch fallback: `getInstituteEventByIdDB(id)` verifying tenant scope.
