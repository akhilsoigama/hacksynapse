# Student Module - Complete Flow (Offline/Online & Data Isolation)

## Objective
Implement robust offline reading, offline data creation (mutation), and strict **Data Isolation (Tenant, Ownership & ID-Wise Access)** for the `student` module.
...
Jab bhi koi user kisi specific record ko uske **ID** se access karega (e.g., `/api/student/:id`), to backend ye verify karega:
...
- Offline mode me, frontend `getStudentDB()` array filter karega: 
...
### A. IndexedDB Setup
**File:** `src/indexDB/student.ts`
- Functions: `getStudentDB()`, `setStudentDB(data)`, `clearStudentDB()`, and `getStudentByIdDB(id)`.
- Schema: `src/indexDB/db.ts` mein `student` table (e.g. `++id, uuid, instituteId, departmentId, createdBy`).

### B. Fetch Hook
**File:** `src/action/student.ts`
- Online hone par API se filtered data aayega.
- Offline hone par `getStudentDB()` ko call karein aur tenant/ID based filter laga kar state update karein.
...
```typescript
student_sync_queue: '++id, uuid, action, status, instituteId, departmentId, createdBy'
```
...
2. **Offline Check:** Agar offline hai, to record `student_sync_queue` me jayega. UI turant update hoga.
3. **Online Mode:** Seedhe `POST /api/student` hit hoga.

### C. Background Sync Logic
**File:** `src/hooks/useStudentOfflineSync.ts`
- `online` event fire hone par, `student_sync_queue` se pending tasks nikal kar bulk me `POST /api/student/sync` par bheje jayenge.
...
- `GET /api/student`
- `GET /api/student/:id` (ID-Wise Access)
- `PUT /api/student/:id` (ID-Wise Update)
- `DELETE /api/student/:id` (ID-Wise Delete)

**Logic Execution in `StudentController`:**
...
## Checklist for Student Module
- [ ] Enforce ID-Wise access control (`show`, `update`, `delete`) based on ownership and institute boundary in backend.
- [ ] Implement Offline ID-wise fetch fallback: `getStudentByIdDB(id)` verifying tenant scope.
