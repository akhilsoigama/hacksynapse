# Assignment Module - Complete Flow (Offline/Online & Data Isolation)

## Objective
Implement robust offline reading, offline data creation (mutation), and strict **Data Isolation (Tenant, Ownership & ID-Wise Access)** for the `assignment` module.
...
Jab bhi koi user kisi specific record ko uske **ID** se access karega (e.g., `/api/assignment/:id`), to backend ye verify karega:
...
- Offline mode me, frontend `getAssignmentDB()` array filter karega: 
...
### A. IndexedDB Setup
**File:** `src/indexDB/assignment.ts`
- Functions: `getAssignmentDB()`, `setAssignmentDB(data)`, `clearAssignmentDB()`, and `getAssignmentByIdDB(id)`.
- Schema: `src/indexDB/db.ts` mein `assignment` table (e.g. `++id, uuid, instituteId, departmentId, createdBy`).

### B. Fetch Hook
**File:** `src/action/assignment.ts`
- Online hone par API se filtered data aayega.
- Offline hone par `getAssignmentDB()` ko call karein aur tenant/ID based filter laga kar state update karein.
...
```typescript
assignment_sync_queue: '++id, uuid, action, status, instituteId, departmentId, createdBy'
```
...
2. **Offline Check:** Agar offline hai, to record `assignment_sync_queue` me jayega. UI turant update hoga.
3. **Online Mode:** Seedhe `POST /api/assignment` hit hoga.

### C. Background Sync Logic
**File:** `src/hooks/useAssignmentOfflineSync.ts`
- `online` event fire hone par, `assignment_sync_queue` se pending tasks nikal kar bulk me `POST /api/assignment/sync` par bheje jayenge.
...
**Routes:**
- `POST /api/assignment/sync`
- `GET /api/assignment`
- `GET /api/assignment/:id` (ID-Wise Access)
- `PUT /api/assignment/:id` (ID-Wise Update)
- `DELETE /api/assignment/:id` (ID-Wise Delete)

**Logic Execution in `AssignmentController`:**
...
## Checklist for Assignment Module
- [ ] Enforce ID-Wise access control (`show`, `update`, `delete`) based on ownership and institute boundary in backend.
- [ ] Implement Offline ID-wise fetch fallback: `getAssignmentByIdDB(id)` verifying tenant scope.
