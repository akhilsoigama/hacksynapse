# Quiz Module - Complete Flow (Offline/Online & Data Isolation)

## Objective
Implement robust offline reading, offline data creation (mutation), and strict **Data Isolation (Tenant, Ownership & ID-Wise Access)** for the `quiz` module.
...
Jab bhi koi user kisi specific record ko uske **ID** se access karega (e.g., `/api/quiz/:id`), to backend ye verify karega:
...
- Offline mode me, frontend `getQuizDB()` array filter karega: 
...
### A. IndexedDB Setup
**File:** `src/indexDB/quiz.ts`
- Functions: `getQuizDB()`, `setQuizDB(data)`, `clearQuizDB()`, and `getQuizByIdDB(id)`.
- Schema: `src/indexDB/db.ts` mein `quiz` table (e.g. `++id, uuid, instituteId, departmentId, createdBy`).

### B. Fetch Hook
**File:** `src/action/quiz.ts`
- Online hone par API se filtered data aayega.
- Offline hone par `getQuizDB()` ko call karein aur tenant/ID based filter laga kar state update karein.
...
```typescript
quiz_sync_queue: '++id, uuid, action, status, instituteId, departmentId, createdBy'
```
...
2. **Offline Check:** Agar offline hai, to record `quiz_sync_queue` me jayega. UI turant update hoga.
3. **Online Mode:** Seedhe `POST /api/quiz` hit hoga.

### C. Background Sync Logic
**File:** `src/hooks/useQuizOfflineSync.ts`
- `online` event fire hone par, `quiz_sync_queue` se pending tasks nikal kar bulk me `POST /api/quiz/sync` par bheje jayenge.
...
- `POST /api/quiz/sync`
- `GET /api/quiz`
- `GET /api/quiz/:id` (ID-Wise Access)
- `PUT /api/quiz/:id` (ID-Wise Update)
- `DELETE /api/quiz/:id` (ID-Wise Delete)

**Logic Execution in `QuizController`:**
...
## Checklist for Quiz Module
- [ ] Enforce ID-Wise access control (`show`, `update`, `delete`) based on ownership and institute boundary in backend.
- [ ] Implement Offline ID-wise fetch fallback: `getQuizByIdDB(id)` verifying tenant scope.
