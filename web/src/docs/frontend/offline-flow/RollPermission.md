# RollPermission Module - Complete Flow (Offline/Online & Data Isolation)

## Objective
Implement robust offline reading, offline data creation (mutation), and strict **Data Isolation (Tenant, Ownership & ID-Wise Access)** for the `RollPermission` module.
...
Jab bhi koi user kisi specific record ko uske **ID** se access karega (e.g., `/api/RollPermission/:id`), to backend ye verify karega:
...
- Offline mode me, frontend `getRollPermissionDB()` array filter karega: 
...
### A. IndexedDB Setup
**File:** `src/indexDB/RollPermission.ts`
- Functions: `getRollPermissionDB()`, `setRollPermissionDB(data)`, `clearRollPermissionDB()`, and `getRollPermissionByIdDB(id)`.
- Schema: `src/indexDB/db.ts` mein `RollPermission` table (e.g. `++id, uuid, instituteId, departmentId, createdBy`).

### B. Fetch Hook
**File:** `src/action/RollPermission.ts`
- Online hone par API se filtered data aayega.
- Offline hone par `getRollPermissionDB()` ko call karein aur tenant/ID based filter laga kar state update karein.
...
```typescript
RollPermission_sync_queue: '++id, uuid, action, status, instituteId, departmentId, createdBy'
```
...
2. **Offline Check:** Agar offline hai, to record `RollPermission_sync_queue` me jayega. UI turant update hoga.
3. **Online Mode:** Seedhe `POST /api/RollPermission` hit hoga.

### C. Background Sync Logic
**File:** `src/hooks/useRollPermissionOfflineSync.ts`
- `online` event fire hone par, `RollPermission_sync_queue` se pending tasks nikal kar bulk me `POST /api/RollPermission/sync` par bheje jayenge.
...
**Routes:**
- `POST /api/RollPermission/sync`
- `GET /api/RollPermission`
- `GET /api/RollPermission/:id` (ID-Wise Access)
- `PUT /api/RollPermission/:id` (ID-Wise Update)
- `DELETE /api/RollPermission/:id` (ID-Wise Delete)

**Logic Execution in `RollPermissionController`:**
...
## Checklist for RollPermission Module
- [ ] Enforce ID-Wise access control (`show`, `update`, `delete`) based on ownership and institute boundary in backend.
- [ ] Implement Offline ID-wise fetch fallback: `getRollPermissionByIdDB(id)` verifying tenant scope.
