# Department Module - Complete Flow (Offline/Online & Data Isolation)

## Objective
Implement robust offline reading, offline data creation (mutation), and strict **Data Isolation (Tenant, Ownership & ID-Wise Access)** for the `department` module.
...
Jab bhi koi user kisi specific record ko uske **ID** se access karega (e.g., `/api/department/:id`), to backend ye verify karega:
...
- Offline mode me, frontend `getDepartmentDB()` array filter karega: 
...
### A. IndexedDB Setup
**File:** `src/indexDB/department.ts`
- Functions: `getDepartmentDB()`, `setDepartmentDB(data)`, `clearDepartmentDB()`, and `getDepartmentByIdDB(id)`.
- Schema: `src/indexDB/db.ts` mein `department` table (e.g. `++id, uuid, instituteId, departmentId, createdBy`).

### B. Fetch Hook
**File:** `src/action/department.ts`
- Online hone par API se filtered data aayega.
- Offline hone par `getDepartmentDB()` ko call karein aur tenant/ID based filter laga kar state update karein.
...
```typescript
department_sync_queue: '++id, uuid, action, status, instituteId, departmentId, createdBy'
```
...
2. **Offline Check:** Agar offline hai, to record `department_sync_queue` me jayega. UI turant update hoga.
3. **Online Mode:** Seedhe `POST /api/department` hit hoga.

### C. Background Sync Logic
**File:** `src/hooks/useDepartmentOfflineSync.ts`
- `online` event fire hone par, `department_sync_queue` se pending tasks nikal kar bulk me `POST /api/department/sync` par bheje jayenge.
...
**Routes:**
- `POST /api/department/sync`
- `GET /api/department`
- `GET /api/department/:id` (ID-Wise Access)
- `PUT /api/department/:id` (ID-Wise Update)
- `DELETE /api/department/:id` (ID-Wise Delete)

**Logic Execution in `DepartmentController`:**
...
## Checklist for Department Module
- [ ] Enforce ID-Wise access control (`show`, `update`, `delete`) based on ownership and institute boundary in backend.
- [ ] Implement Offline ID-wise fetch fallback: `getDepartmentByIdDB(id)` verifying tenant scope.
