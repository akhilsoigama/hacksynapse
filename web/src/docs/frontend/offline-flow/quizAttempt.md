# QuizAttempt Module - Complete Flow (Offline/Online & Data Isolation)

## Objective
Implement robust offline reading, offline data creation (mutation), and strict **Data Isolation (Tenant, Ownership & ID-Wise Access)** for the `quizAttempt` module.
...
Jab bhi koi user kisi specific record ko uske **ID** se access karega (e.g., `/api/quizAttempt/:id`), to backend ye verify karega:
...
- Offline mode me, frontend `getQuizAttemptDB()` array filter karega: 
...
### A. IndexedDB Setup
**File:** `src/indexDB/quizAttempt.ts`
- Functions: `getQuizAttemptDB()`, `setQuizAttemptDB(data)`, `clearQuizAttemptDB()`, and `getQuizAttemptByIdDB(id)`.
- Schema: `src/indexDB/db.ts` mein `quizAttempt` table (e.g. `++id, uuid, instituteId, departmentId, createdBy`).

### B. Fetch Hook
**File:** `src/action/quizAttempt.ts`
- Online hone par API se filtered data aayega.
- Offline hone par `getQuizAttemptDB()` ko call karein aur tenant/ID based filter laga kar state update karein.
...
```typescript
quizAttempt_sync_queue: '++id, uuid, action, status, instituteId, departmentId, createdBy'
```
...
2. **Offline Check:** Agar offline hai, to record `quizAttempt_sync_queue` me jayega. UI turant update hoga.
3. **Online Mode:** Seedhe `POST /api/quizAttempt` hit hoga.

### C. Background Sync Logic
**File:** `src/hooks/useQuizAttemptOfflineSync.ts`
- `online` event fire hone par, `quizAttempt_sync_queue` se pending tasks nikal kar bulk me `POST /api/quizAttempt/sync` par bheje jayenge.
...
**Routes:**
- `POST /api/quizAttempt/sync`
- `GET /api/quizAttempt`
- `GET /api/quizAttempt/:id` (ID-Wise Access)
- `PUT /api/quizAttempt/:id` (ID-Wise Update)
- `DELETE /api/quizAttempt/:id` (ID-Wise Delete)

**Logic Execution in `QuizAttemptController`:**
...
## Checklist for QuizAttempt Module
- [ ] Enforce ID-Wise access control (`show`, `update`, `delete`) based on ownership and institute boundary in backend.
- [ ] Implement Offline ID-wise fetch fallback: `getQuizAttemptByIdDB(id)` verifying tenant scope.
