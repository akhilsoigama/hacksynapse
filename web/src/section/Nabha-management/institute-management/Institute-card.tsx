...
interface InstituteCardProps {
  ...
  onViewDetails: (institute: IInstitute) => void;
  index: number;
}
    ...
    >
      <div className="p-5">
        <div className="flex justify-between items-start">
          <div>
            <h3 className="text-lg font-semibold text-gray-800">{institute.instituteName}</h3>
            <p className="text-sm text-gray-500 mt-1">{institute.instituteCode}</p>
          </div>
          <span className={`px-2.5 py-1 text-xs font-medium rounded-full ${institute.isActive ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
        ...
        <div className="mt-4 flex items-center text-sm text-gray-600">
          ...
          <span>{institute.instituteCity}, {institute.instituteState}</span>
        </div>
