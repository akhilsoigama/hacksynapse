...
const CAREER_ROADMAP_SUB_CATEGORIES = CATEGORY_MAP['Career Roadmap'] || [
    'Resume Building',
    'Interview Skills',
    'Job Search',
    'Networking',
...
const getSubCategoryIcon = (subCat?: string) => {
    switch (subCat) {
        case 'Resume Building':
            return <Description fontSize="small" className="text-violet-500" />
        case 'Interview Skills':
            return <RecordVoiceOver fontSize="small" className="text-amber-500" />
        case 'Job Search':
            return <Work fontSize="small" className="text-blue-500" />
        case 'Networking':
            return <Hub fontSize="small" className="text-emerald-500" />
        case 'LinkedIn Profile':
            return <LinkedIn fontSize="small" className="text-sky-500" />
        default:
            return <AltRoute fontSize="small" className="text-violet-500" />
    }
}
...
const getSubCategoryBadgeStyle = (subCat?: string, isDark?: boolean) => {
    if (isDark) {
        switch (subCat) {
            case 'Resume Building': return 'bg-violet-950/40 text-violet-300 border-violet-800/40'
            case 'Interview Skills': return 'bg-amber-950/40 text-amber-300 border-amber-800/40'
            case 'Job Search': return 'bg-blue-950/40 text-blue-300 border-blue-800/40'
            case 'Networking': return 'bg-emerald-950/40 text-emerald-300 border-emerald-800/40'
            case 'LinkedIn Profile': return 'bg-sky-950/40 text-sky-300 border-sky-800/40'
            default: return 'bg-slate-800 text-slate-300 border-slate-700'
        }
    } else {
        switch (subCat) {
            case 'Resume Building': return 'bg-violet-50 text-violet-700 border-violet-200'
            case 'Interview Skills': return 'bg-amber-50 text-amber-700 border-amber-200'
            case 'Job Search': return 'bg-blue-50 text-blue-700 border-blue-200'
            case 'Networking': return 'bg-emerald-50 text-emerald-700 border-emerald-200'
            case 'LinkedIn Profile': return 'bg-sky-50 text-sky-700 border-sky-200'
...
const CareerRoadmapList = () => {
    ...
    const canonicalSub = rawSubParam ? (getCanonicalSubCategory('Career Roadmap', rawSubParam) || rawSubParam) : 'All'
    ...
    useEffect(() => {
        ...
        const resolved = currentRaw ? (getCanonicalSubCategory('Career Roadmap', currentRaw) || currentRaw) : 'All'
    ...
    const filteredCourses = useMemo(() => {
        ...
        return apiCourses.filter((course) => {
            if (!isCategoryMatch(course.category, 'Career Roadmap')) {
                return false
            }
            ...
            <div className="max-w-7xl mx-auto px-4 py-8">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                    <div>
                        ...
                        <p className={cn(
                            ...
                            Build professional resumes, excel in interviews, optimize job search, and master LinkedIn networking
                        </p>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                        <ParticleButton
                            ...
                            onClick={() => {
                                ...
                                navigate(`/dashboard/skills/rag/new?category=Career+Roadmap${subParam}`)
                            }}
                        >
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                    ...
                    <div className={cn(
                        ...
                        <p className={cn("text-xl font-bold truncate", isDark ? 'text-violet-400' : 'text-violet-600')}>
                    ...
                    <div className={cn(
                        ...
                        <p className={cn("text-2xl font-bold text-violet-500", isDark ? 'text-violet-400' : 'text-violet-600')}>
                            Video & Interactive
                        </p>
                        <p className={cn("text-xs font-medium uppercase tracking-wider mt-0.5", isDark ? 'text-gray-400' : 'text-gray-500')}>
                            Format
                ...
                <div className="space-y-4 mb-8">
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
                        <div className="relative flex-1 max-w-md">
                            ...
                            <input
                                ...
                                placeholder="Search inside Career Roadmap..."
                                className={cn(
                                    "w-full pl-9 pr-4 py-2 rounded-xl text-sm border outline-none transition-all",
                                    isDark
                                        ? "bg-slate-900 border-slate-700 text-slate-100 placeholder-slate-500 focus:border-violet-500"
                                        : "bg-white border-slate-300 text-slate-900 placeholder-slate-400 focus:border-violet-500 shadow-sm"
                                )}
                            />
                        </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        {['All', ...CAREER_ROADMAP_SUB_CATEGORIES].map((subCat) => {
                ...
                {!coursesLoading && filteredCourses.length === 0 && (
                    <div className={cn(
                        ...
                        <div className="w-12 h-12 mx-auto rounded-full bg-violet-500/10 text-violet-400 flex items-center justify-center mb-3">
                        ...
                        <h3 className={cn("text-lg font-bold mb-1", isDark ? "text-white" : "text-slate-900")}>
                            {selectedSubCategory !== 'All'
                                ...
                                : searchQuery
                                    ...
                                    : "No Career Roadmap courses created yet."}
                        </h3>
                        <p className="text-sm max-w-md mx-auto mb-6">
                            {selectedSubCategory !== 'All'
                                ? `No courses have been created under Career Roadmap → "${selectedSubCategory}". Click below to create one.`
                                : searchQuery
                                    ? "Try searching for a different keyword or reset your filter."
                                    : "Courses created in Course Creation under Career Roadmap will appear here automatically."}
                        </p>
                        ...
                        <div className="flex items-center justify-center gap-3">
                            <ParticleButton
                                ...
                                className={cn(
                                    ...
                                    "bg-violet-500 hover:bg-violet-600 shadow-md shadow-violet-500/20"
                                )}
                                successDuration={600}
                                onClick={() => {
                                    const subParam = selectedSubCategory !== 'All' ? `&subCategory=${encodeURIComponent(selectedSubCategory)}` : ''
                        ...
                        {filteredCourses.map((course, index) => (
                            <div
                                ...
                                className={cn(
                                    ...
                                    isDark
                                        ? "border-slate-800 bg-slate-900/50 hover:border-violet-500/30 hover:bg-slate-900/80 hover:shadow-xl hover:shadow-violet-500/5"
                                        : "border-slate-200 bg-white hover:shadow-lg hover:shadow-violet-100/20",
                                ...
                                <div className="relative flex flex-col space-y-3">
                                    <div className="flex items-center justify-between">
                                        <div className={cn(
                                            ...
                                            isDark ? "bg-slate-800 text-slate-300" : "bg-slate-100 text-slate-600 group-hover:bg-violet-50"
                                        )}>
                                        ...
                                        <div className="flex items-center gap-2">
                                            <span className={cn(
                                                ...
                                                {course.subCategory || 'Career Roadmap'}
                                            </span>
                                            <span className={cn(
                                                ...
                                                course.videoType === 'youtube'
                                                    ...
                                                    : isDark ? "bg-violet-950/30 text-violet-400 border-violet-800/40" : "bg-violet-50 text-violet-600 border-violet-200"
                                            )}>
                                    ...
                                    <div className="space-y-1.5">
                                        ...
                                        <p className={cn("text-xs leading-snug line-clamp-2", isDark ? "text-slate-400" : "text-slate-600")}>
                                            {course.description || 'Comprehensive Career Roadmap course module.'}
                                        </p>
                                    </div>
                                            ...
                                            {course.tags.slice(0, 3).map((tag, i) => (
                                                <span
                                                    ...
                                                    className={cn(
                                                        ...
                                                        isDark ? "bg-slate-800 text-slate-300 border-slate-700" : "bg-slate-100 text-slate-600 border-slate-200 group-hover:bg-violet-50"
                                                    )}
                                                >
                                                    #{tag}
                                                </span>
                                    ...
                                    <div className="mt-1 text-xs flex items-center gap-1 text-violet-500 font-medium">
...
export { CareerRoadmapList as CareerRoadmap }
