...
const DIGITAL_SKILLS_SUB_CATEGORIES = CATEGORY_MAP['Digital Skills'] || [
    'Mobile Banking',
    'UPI Payments',
    'Online Safety',
    'Social Media',
...
const getSubCategoryIcon = (subCat?: string) => {
    switch (subCat) {
        case 'Mobile Banking':
            return <AccountBalanceWallet fontSize="small" className="text-teal-500" />
        case 'UPI Payments':
            return <QrCodeScanner fontSize="small" className="text-cyan-500" />
        case 'Online Safety':
            return <Security fontSize="small" className="text-rose-500" />
        case 'Social Media':
            return <Share fontSize="small" className="text-blue-500" />
        case 'E-Commerce':
            return <ShoppingCart fontSize="small" className="text-amber-500" />
        default:
            return <PhoneAndroid fontSize="small" className="text-teal-500" />
    }
}
...
const getSubCategoryBadgeStyle = (subCat?: string, isDark?: boolean) => {
    if (isDark) {
        switch (subCat) {
            case 'Mobile Banking': return 'bg-teal-950/40 text-teal-300 border-teal-800/40'
            case 'UPI Payments': return 'bg-cyan-950/40 text-cyan-300 border-cyan-800/40'
            case 'Online Safety': return 'bg-rose-950/40 text-rose-300 border-rose-800/40'
            case 'Social Media': return 'bg-blue-950/40 text-blue-300 border-blue-800/40'
            case 'E-Commerce': return 'bg-amber-950/40 text-amber-300 border-amber-800/40'
            default: return 'bg-slate-800 text-slate-300 border-slate-700'
        }
    } else {
        switch (subCat) {
            case 'Mobile Banking': return 'bg-teal-50 text-teal-700 border-teal-200'
            case 'UPI Payments': return 'bg-cyan-50 text-cyan-700 border-cyan-200'
            case 'Online Safety': return 'bg-rose-50 text-rose-700 border-rose-200'
            case 'Social Media': return 'bg-blue-50 text-blue-700 border-blue-200'
            case 'E-Commerce': return 'bg-amber-50 text-amber-700 border-amber-200'
...
const DigitalSkillList = () => {
    ...
    const canonicalSub = rawSubParam ? (getCanonicalSubCategory('Digital Skills', rawSubParam) || rawSubParam) : 'All'
    ...
    useEffect(() => {
        ...
        const resolved = currentRaw ? (getCanonicalSubCategory('Digital Skills', currentRaw) || currentRaw) : 'All'
    ...
    const filteredCourses = useMemo(() => {
        ...
        return apiCourses.filter((course) => {
            if (!isCategoryMatch(course.category, 'Digital Skills')) {
                return false
            }
            ...
            <div className="max-w-7xl mx-auto px-4 py-8">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                    <div>
                        ...
                        <p className={cn(
                            ...
                            Master mobile banking, UPI payments, online safety, social media, and e-commerce
                        </p>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                        <ParticleButton
                            ...
                            onClick={() => {
                                ...
                                navigate(`/dashboard/skills/rag/new?category=Digital+Skills${subParam}`)
                            }}
                        >
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                    ...
                    <div className={cn(
                        ...
                        <p className={cn("text-xl font-bold truncate", isDark ? 'text-teal-400' : 'text-teal-600')}>
                    ...
                    <div className={cn(
                        ...
                        <p className={cn("text-2xl font-bold text-teal-500", isDark ? 'text-teal-400' : 'text-teal-600')}>
                            Video & Interactive
                        </p>
                        <p className={cn("text-xs font-medium uppercase tracking-wider mt-0.5", isDark ? 'text-gray-400' : 'text-gray-500')}>
                            Format
                ...
                <div className="space-y-4 mb-8">
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
                        <div className="relative flex-1 max-w-md">
                            ...
                            <input
                                ...
                                placeholder="Search inside Digital Skills..."
                                className={cn(
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        {['All', ...DIGITAL_SKILLS_SUB_CATEGORIES].map((subCat) => {
                        ...
                        <h3 className={cn("text-lg font-bold mb-1", isDark ? "text-white" : "text-slate-900")}>
                            {selectedSubCategory !== 'All'
                                ...
                                : searchQuery
                                    ...
                                    : "No Digital Skills courses created yet."}
                        </h3>
                        <p className="text-sm max-w-md mx-auto mb-6">
                            {selectedSubCategory !== 'All'
                                ? `No courses have been created under Digital Skills → "${selectedSubCategory}". Click below to create one.`
                                : searchQuery
                                    ? "Try searching for a different keyword or reset your filter."
                                    : "Courses created in Course Creation under Digital Skills will appear here automatically."}
                        </p>
                                ...
                                <div className="relative flex flex-col space-y-3">
                                    <div className="flex items-center justify-between">
                                        <div className={cn(
                                            ...
                                            isDark ? "bg-slate-800 text-slate-300" : "bg-slate-100 text-slate-600 group-hover:bg-teal-50"
                                        )}>
                                        ...
                                        <div className="flex items-center gap-2">
                                            <span className={cn(
                                                ...
                                                {course.subCategory || 'Digital Skills'}
                                            </span>
                                            <span className={cn(
                                    ...
                                    <div className="space-y-1.5">
                                        ...
                                        <p className={cn("text-xs leading-snug line-clamp-2", isDark ? "text-slate-400" : "text-slate-600")}>
                                            {course.description || 'Comprehensive Digital Skills course module.'}
                                        </p>
                                    </div>
                                            ...
                                            {course.tags.slice(0, 3).map((tag, i) => (
                                                <span
                                                    ...
                                                    className={cn(
                                                        ...
                                                        isDark ? "bg-slate-800 text-slate-300 border-slate-700" : "bg-slate-100 text-slate-600 border-slate-200 group-hover:bg-teal-50"
                                                    )}
                                                >
                                                    #{tag}
                                                </span>
...
export { DigitalSkillList as DigitalSkill }
export default DigitalSkillList